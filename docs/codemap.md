...
| WebAssembly chained exits | A block end's slot check, stop and budget, and the `call_indirect` transfer through the host table; the per-block slot cap and a relowered block's slot reuse | `src/x86port/jit_wasm_chain.{h,c}` | `x86p_wasm_chain_emit`, `X86pWasmChainUse` | `jit_chain.h` |
