...
| Binary64 x87 arithmetic | The consumer-selected mode that computes x87 add, subtract, multiply and divide in binary64 on a host whose register file is the ext80 encoding, answering only round-to-nearest zeros and normals and refusing everything else to the exact path; the WebAssembly backend computes the same set in the block | `src/x86port/x87_double_arith.{h,c}`, `src/x86port/jit_wasm_x87_arith.{h,c}` | `x86p_x87_set_double_arith`, `x86p_ext80_double_arith` | — |
| JIT x87 helpers | The per-instruction x87 helpers translated code calls, in the register file's storage type from guest memory to guest memory, shared by the WebAssembly and AArch64 backends; never through the `long double` API, which is software binary128 on those hosts | `src/x86port/jit_x87_helpers.{h,c}` | `x86p_jit_x87_load_bits`, `x86p_jit_x87_arith_mem_bits`, `x86p_jit_x87_arith_reg`, `x86p_jit_x87_store_bytes`, `x86p_jit_x87_copy` | — |
...
| Flag liveness | Dead flag store elimination shared by the native backends: whether a flag writer's tuple is overwritten, within the block, before anything can read it or fault | `src/x86port/jit_flag_liveness.{h,c}` | `x86p_jit_flag_write_is_dead` | — |
...
| ARM64 backend | Block orchestration, arithmetic/lazy-flag lowering, x87 lowering (helper calls, with inline binary64 fast paths for ordinary arithmetic, float loads/stores, compares and register moves in `jit_arm64_x87_inline`), and host encoding with shared register/layout primitives | `src/x86port/jit_arm64.c`, `src/x86port/jit_arm64_internal.h`, `src/x86port/jit_arm64_integer.{h,c}`, `src/x86port/jit_arm64_{alu,branch,simd,x87_register}.c`, `src/x86port/jit_arm64_x87.{h,c}`, `src/x86port/jit_arm64_x87_inline.{h,c}`, `src/x86port/emit_arm64.{h,c}` | `x86p_jit_translate` | `migration.md` |
...
| WebAssembly chained exits | A block end's slot check, stop and budget, the tail-call transfer through the trampoline module that alone imports the host table, and the direct call to a block of the same shared module; the per-block slot cap and a relowered block's slot reuse | `src/x86port/jit_wasm_chain.{h,c}` | `x86p_wasm_chain_emit`, `X86pWasmChainUse`, `x86p_wasm_chain_trampoline` | `jit_chain.h` |
