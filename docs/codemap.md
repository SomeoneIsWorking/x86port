...
| Guest memory | Borrowed contiguous desktop spans or exact sparse guest mappings; scalar and byte access, fault admission, native span resolution and mapping lifetime | `src/x86port/cpu.h`, `src/x86port/memory.c`, `src/x86port/memory_sparse.{h,c}` | `X86pMem`, `x86p_mem_resolve`, `x86p_sparse_map`, `x86p_mem_read_bytes` | `memory.md` |
