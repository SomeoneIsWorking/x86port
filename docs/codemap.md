...
| x64 backend | x86-32 basic-block lowering, host-call ABI ownership, guest memory operands, the write-through guest register cache, inline x87 with its host-stack mirror, and x86-64 machine-code emission | `src/x86port/jit_x64.{h,c}`, `src/x86port/jit_x64_internal.h`, `src/x86port/jit_x64_abi.h`, `src/x86port/jit_x64_{alu,branch,cond,mem,simd,x87_register}.c`, `src/x86port/jit_x64_gpr.{h,c}`, `src/x86port/jit_x64_x87.{h,c}`, `src/x86port/jit_x64_x87_inline.{h,c}`, `src/x86port/emit_x64.{h,c}` | `x86p_jit_translate` | `migration.md` |
