...
| x87 guest memory formats | Validated floating and signed-integer guest memory conversion shared by the test oracle and generated-code adapters, from an address or from operand bits a backend loaded itself | `src/x86port/x87_memory.{h,c}` | `x86p_x87_read_value`, `x86p_x87_write_value`, `x86p_x87_reg_from_operand_bits` | — |
