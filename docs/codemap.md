...
| Binary64 x87 arithmetic | The consumer-selected mode that computes x87 add, subtract, multiply and divide in binary64 on a host whose register file is the ext80 encoding, answering only round-to-nearest zeros and normals and refusing everything else to the exact path | `src/x86port/x87_double_arith.{h,c}` | `x86p_x87_set_double_arith`, `x86p_ext80_double_arith` | — |
