...
| WebAssembly encoder | The WebAssembly binary format only: module sections, padded size patching, LEB128, and the instruction encodings a lowering backend needs. No guest state and no lowering yet; a `jit_wasm` backend does not exist | `src/x86port/emit_wasm.{h,c}` | `x86p_wasm_module_begin`, `x86p_wasm_section_begin`, `x86p_wasm_ok` | `migration.md` |
