...
| x64 backend | x86-32 basic-block lowering, host-call ABI ownership, and x86-64 machine-code emission | `src/x86port/jit_x64.{h,c}`, `src/x86port/jit_x64_internal.h`, `src/x86port/jit_x64_abi.h`, `src/x86port/jit_x64_x87.{h,c}`, `src/x86port/emit_x64.{h,c}` | `x86p_jit_translate` | `migration.md` |
...
| Framework verification | Locked host build/test orchestration, unit, hardware-oracle, differential, product-only runtime, link-boundary, and architecture tests | `tests/`, `tools/verify.py`, `tools/verify_product_boundary.py`, `tools/x86port_checks/` | `tools/verify.py`, CTest targets | `project-state.md` |
