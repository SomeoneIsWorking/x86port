...
static unsigned cases, failures, precision_refusals;
...
static void check_x87_case(const uint8_t *program, size_t length, X86pCpu initial, int fault) {
  if (x87_values_are_emittable()) {
    check_case(program, length, initial, fault);
    return;
  }
  uint8_t guest[4096] = {0};
  X86pMem mem = {.host = guest, .lo = 0x10000, .size = sizeof guest};
  X86pJitBlock block;
  JcCodeRegion code;
  char reason[256] = {0};
  ...
  const X86pJitStatus status =
      x86p_jit_translate(&mem, initial.eip, code.write, code.size, &block, reason, sizeof reason);
  if (status != kX86pJitUnsupportedAtEntry || !reason[0] || memcmp(guest, program, length)) {
    printf("x87 precision refusal failed: status %d, reason %s\n", status, reason);
    failures++;
  } else {
    precision_refusals++;
  }
...
int main(void) {
  ...
  for (unsigned depth = 0; depth <= 8; depth++) {
    ...
    /* Empty ST(0) hid a dispatch fallthrough from FWAIT into FLD. Every depth,
     * including a full stack, must preserve the complete CPU state. */
    const uint8_t wait[] = {0x9B};
    check_case(wait, sizeof wait, cpu, 0);
    for (unsigned i = 0; i < sizeof memory_forms / sizeof memory_forms[0]; i++) {
      ...
      bad[2] = 0xFF;
      bad[3] = 0x0F;
      check_x87_case(memory_forms[i], 6, cpu, 0);
      const int writes = i >= 3 && i <= 5;
      check_x87_case(bad, 6, cpu, !writes || depth != 0);
    }
