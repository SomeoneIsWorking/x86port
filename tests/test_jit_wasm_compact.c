...
static int publish_singly(X86pWasmArena *arena, const X86pMem *mem, X86pWasmCompactBlock *out, unsigned count) {
  ...
  for (i = 0; i < count; ++i) {
    ...
    bytes =
        x86p_jit_translate_batch(mem, &eip, 1u, g_buffer, sizeof g_buffer, NULL, NULL, &block, reason, sizeof reason);
    if (bytes == 0) {
...
static void test_four_modules_become_one(void) {
  ...
  bind(&arena, &stub, &host);
  {
    ...
    check("the batch is compacted",
          x86p_wasm_compact(
              &arena, &mem, g_buffer, sizeof g_buffer, NULL, NULL, blocks, BLOCKS, &result, reason, sizeof reason),
          1);
  }
