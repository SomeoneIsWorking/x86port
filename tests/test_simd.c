...
static const uint8_t kInputs[][32] = {
    ...
    /* A NaN in BOTH operands of lanes 0 and 1 (a quiet one first, then a
       signalling one), a signalling NaN in the source alone in lane 2, and
       infinity against zero in lane 3. C does not say which NaN `a * b`
       returns and a compiler may commute it; SSE returns the destination's. */
    {0x01, 0x00, 0xC0, 0x7F, 0x01, 0x00, 0x80, 0x7F, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0x7F,
     0x02, 0x00, 0xC0, 0xFF, 0x03, 0x00, 0xC0, 0xFF, 0x05, 0x00, 0x80, 0xFF, 0x00, 0x00, 0x00, 0x00},
};
...
int main(void) {
  ...
  return g_failed ? 1 : 0;
#else
  printf("SKIP: x86-64 hardware oracle unavailable; 0 host instructions executed, MMX and SSE unchecked.\n");
  return 77;
#endif
}
