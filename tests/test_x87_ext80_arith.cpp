...
X86pExt80 oracle(X86pExt80 x, X86pExt80 y, uint16_t *flags) {
  X86pX87Reg a;
  X86pX87Reg b;
  X86pX87Reg r;
  X86pExt80 out;
  ...
  *flags = 0;
  r = x86p_x87_software_arith_raw(X86P_X87_CW_INIT, kX86pX87Mul, a, b, flags);
  out.signif = r.signif;
...
#if HAVE_ORACLE
void differ(X86pExt80 x, X86pExt80 y, const char *what) {
  X86pExt80 got;
  X86pExt80 want;
  uint16_t flags = 0;
  ...
  g_taken++;
  g_checks++;
  want = oracle(x, y, &want_flags);
  if (got.signif != want.signif || got.sign_exp != want.sign_exp || (oracle_reports_flags && flags != want_flags)) {
    std::printf("FAIL %s %04x:%016llx * %04x:%016llx: got %04x:%016llx f%03x %s %04x:%016llx f%03x\n",
                what,
                x.sign_exp,
...
int main() {
...
#if HAVE_ORACLE
  {
    ...
    int i;
    /* Full-width significands: every product rounds, so this is the sweep
    ...
    for (i = 0; i < 200000; i++) {
      ...
      const uint16_t ea = (uint16_t)(0x3000u + (next(&state) & 0x1FFFu));
      const uint16_t eb = (uint16_t)(0x3000u + (next(&state) & 0x1FFFu));
      differ(ext80(ea, sa), ext80(eb, sb), "wide sweep");
    }
    /* Exact ties: a product whose dropped half is exactly one half, which is
    ...
    for (i = 0; i < 100000; i++) {
      ...
      const uint16_t ea = (uint16_t)(0x3FF0u + (next(&state) & 0xFu));
      const uint16_t eb = (uint16_t)(0x3FF0u + (next(&state) & 0xFu));
      differ(ext80(ea, sa), ext80(eb, sb), "tie sweep");
    }
    ...
    for (i = 0; i < 100000; i++) {
      const uint64_t sa = next(&state) | (1ull << 63);
      const uint64_t sb = next(&state) | (1ull << 63);
      const uint16_t ea = (uint16_t)(1u + (next(&state) % 0x7FFEu));
      const uint16_t eb = (uint16_t)(1u + (next(&state) % 0x7FFEu));
      differ(ext80(ea, sa), ext80(eb, sb), "edge sweep");
    }
    /* Binary32-derived operands: what a game supplies, and a population whose
       products mostly do not round at all. */
    for (i = 0; i < 100000; i++) {
      ...
      differ(x86p_ext80_from_f32_bits(ab), x86p_ext80_from_f32_bits(bb), "binary32 sweep");
    }
  }
  ...
  /* The comparator must be able to fail. */
  {
    ...
    if (!x86p_ext80_mul_ordinary(X86P_X87_CW_INIT,
      ...
      want = oracle(ext80(one, 0x8000000000000000ull), ext80(one, 0xC000000000000000ull), &want_flags);
  ...
  std::printf("x87 ordinary ext80 multiply (%s oracle): %lld check(s), %lld taken, %lld refused (%.1f%% of %lld), "
              "%d failure(s)\n",
              oracle_name,
              g_checks,
              g_taken,
              ...
              g_failures);
#else
  std::printf("x87 ordinary ext80 multiply: table only, no oracle on this host: %lld check(s), %d failure(s)\n",
              g_checks,
              g_failures);
#endif
