...
void emit_simd_bits(BlockCtx *c, const X86pInsn *insn, uint32_t pc) {
  if (insn->simd == kX86pSimdEmms) {
    ...
    x86p_a64_emit_mov_x_imm64(c->e, kA64X9, (uint64_t)(uintptr_t)&x86p_x87_emms);
  ...
  if (insn->simd == kX86pSimdMovlps || insn->simd == kX86pSimdMovhps || insn->simd == kX86pSimdMovhlps ||
    ...
    x86p_a64_emit_load32(c->e, kA64X0, memory_src ? HOSTPTR_REG : CPU_REG, from);
    x86p_a64_emit_load32(c->e, kA64X1, memory_src ? HOSTPTR_REG : CPU_REG, from + 4);
    x86p_a64_emit_store32(c->e, memory_dst ? HOSTPTR_REG : CPU_REG, to, kA64X0);
    x86p_a64_emit_store32(c->e, memory_dst ? HOSTPTR_REG : CPU_REG, to + 4, kA64X1);
    return;
  }
  if (insn->simd == kX86pSimdMovss) {
    x86p_a64_emit_load32(c->e, kA64X1, memory_src ? HOSTPTR_REG : CPU_REG, memory_src ? 0 : source);
    x86p_a64_emit_store32(c->e, memory_dst ? HOSTPTR_REG : CPU_REG, memory_dst ? 0 : dst, kA64X1);
    if (memory_src) {
      x86p_a64_emit_mov_w_imm32(c->e, kA64X1, 0);
      for (int32_t lane = 4; lane < 16; lane += 4) {
        x86p_a64_emit_store32(c->e, CPU_REG, dst + lane, kA64X1);
      }
    }
    return;
  }
  void (*arithmetic)(void *, const void *) = insn->simd == kX86pSimdAddps   ? x86p_simd_addps
                                             ...
                                             : insn->simd == kX86pSimdDivps ? x86p_simd_divps
                                                                            : NULL;
  if (arithmetic) {
    x86p_a64_emit_lea64(c->e, kA64X0, CPU_REG, dst);
    x86p_a64_emit_lea64(c->e, kA64X1, memory_src ? HOSTPTR_REG : CPU_REG, memory_src ? 0 : source);
    x86p_a64_emit_mov_x_imm64(c->e, kA64X9, (uint64_t)(uintptr_t)arithmetic);
    x86p_a64_emit_blr(c->e, kA64X9);
    return;
  }
  if (insn->simd == kX86pSimdShufps) {
    const int lanes[4] = {kA64X0, kA64X1, kA64X2, kA64X3};
    ...
    for (int32_t lane = 0; lane < 4; lane++) {
      x86p_a64_emit_store32(c->e, CPU_REG, dst + lane * 4, lanes[lane]);
    }
    return;
  }
  for (int32_t lane = 0; lane < 16; lane += 4) {
    x86p_a64_emit_load32(c->e, kA64X1, memory_src ? HOSTPTR_REG : CPU_REG, memory_src ? lane : source + lane);
    if (insn->simd != kX86pSimdMovaps) {
      x86p_a64_emit_load32(c->e, kA64X0, CPU_REG, dst + lane);
      if (insn->simd == kX86pSimdAndnps) {
        x86p_a64_emit_alu_w_imm(c->e, kA64Eor, kA64X0, UINT32_MAX);
      }
      x86p_a64_emit_alu_w_w(c->e,
                            insn->simd == kX86pSimdOrps    ? kA64Orr
                            : insn->simd == kX86pSimdXorps ? kA64Eor
                                                           : kA64And,
                            kA64X1,
                            kA64X0);
    }
    x86p_a64_emit_store32(c->e, memory_dst ? HOSTPTR_REG : CPU_REG, memory_dst ? lane : dst + lane, kA64X1);
  }
}
