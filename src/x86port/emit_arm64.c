...
static void orr_shifted_reg(X86pA64Emit *e, int sf, unsigned opc, X86pA64Reg rd, X86pA64Reg rn, X86pA64Reg rm) {
  uint32_t word =
      ((uint32_t)sf << 31) | (opc << 29) | (0x0Au << 24) | ((uint32_t)rm << 16) | ((uint32_t)rn << 5) | (uint32_t)rd;
  put32(e, word);
}
...
static void
add_sub_imm(X86pA64Emit *e, int sf, int is_sub, X86pA64Reg rd, X86pA64Reg rn, uint32_t imm12, unsigned shift12) {
...
static void
ldst_unsigned(X86pA64Emit *e, uint32_t base_word, X86pA64Reg rt, X86pA64Reg rn, int32_t disp, int size_bytes) {
  uint32_t imm;
...
void x86p_a64_emit_cset_w(X86pA64Emit *e, X86pA64Cond cc, X86pA64Reg dst) {
  ...
  uint32_t word =
      A64_CSEL_BASE | (31u << 16) | (inv << 12) | (1u << 10) /* op2=01: CSINC */ | (31u << 5) | (uint32_t)dst;
  put32(e, word);
}
